...
{
    void * R=LD_Object_Pool_Init(&Graph->Pool,Object_Size+(sizeof(void*)*Max_Grade)+sizeof(uintptr_t),Order);
...
struct LD_Fixed_Graph * LD_Fixed_Graph_Create(uintptr_t Object_Size, uintptr_t Max_Grade,uintptr_t Order)
{
	...
	(struct LD_Fixed_Graph*) malloc(sizeof(struct LD_Object_Pool));
    if(RETURN ==NULL){return NULL;}
    RETURN = LD_Fixed_Graph_Init(RETURN,Object_Size,Max_Grade,Order);
    if(RETURN ==NULL){free(RETURN);}
    return RETURN;
}
