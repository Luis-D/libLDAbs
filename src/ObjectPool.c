...
#ifndef LD_OBJECTPOOL_DEFAULT_ARENA_SIZE 
    #define LD_OBJECTPOOL_DEFAULT_ARENA_SIZE 4096 //<- 4kb
#endif
...
struct LD_ObjectPool * LD_ObjectPool_Init_EXT(struct LD_ObjectPool * Pool,
			    	size_t Object_Count,size_t Object_Size)
{
...
struct LD_ObjectPool * LD_ObjectPool_Init(struct LD_ObjectPool * Pool, size_t Object_Size)
{
    size_t Object_Count = LD_OBJECTPOOL_DEFAULT_ARENA_SIZE/(Object_Size);
    return LD_ObjectPool_Init_EXT(Pool,Object_Count,Object_Size);
}

struct LD_ObjectPool * LD_ObjectPool_Create_EXT(size_t Object_Count,size_t Object_Size)
{
    ...
    if(LD_ObjectPool_Init_EXT(Pool,Object_Count,Object_Size)==NULL){return NULL;}

    return Pool;
}

struct LD_ObjectPool * LD_ObjectPool_Create(size_t Object_Size)
{
    size_t Object_Count = LD_OBJECTPOOL_DEFAULT_ARENA_SIZE/(Object_Size);
    return LD_ObjectPool_Create_EXT(Object_Count,Object_Size);
}
